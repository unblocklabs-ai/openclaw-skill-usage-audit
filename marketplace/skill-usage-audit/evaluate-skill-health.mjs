...
import { buildSkillRootSpecs, resolveBundledSkillsRoot, resolveHomePath as resolveHome } from "./skill-roots.mjs";
...
async function buildWorkspaceRoots() {
  const workspaceRoot = resolve(__dirname, "../..");
  const bundledRoot = resolveBundledSkillsRoot({ moduleDir: __dirname });
  const specs = await buildSkillRootSpecs({
    workspaceDir: workspaceRoot,
    bundledRoot,
    includeBundled: Boolean(bundledRoot),
  });
  const rootsBySource = new Map();
  for (const spec of specs) {
    if (!rootsBySource.has(spec.source)) rootsBySource.set(spec.source, []);
    rootsBySource.get(spec.source).push(spec.root);
  }

  return {
    workspaceSkills: rootsBySource.get("openclaw-workspace")?.[0],
    projectAgentSkills: rootsBySource.get("agents-skills-project")?.[0],
    personalAgentSkills: rootsBySource.get("agents-skills-personal")?.[0],
    openclawSkills: rootsBySource.get("openclaw-managed")?.[0],
    bundledSkills: rootsBySource.get("openclaw-bundled")?.[0],
    extensionSkillsDir: resolveHome("~/.openclaw/extensions"),
    pluginSkills: rootsBySource.get("openclaw-extra")?.find((root) => root.endsWith(`${sep}.openclaw${sep}plugin-skills`)),
    codexSkills: rootsBySource.get("codex")?.[0],
    extraSkillDirs: (rootsBySource.get("openclaw-extra") || []).filter((root) => !root.endsWith(`${sep}.openclaw${sep}plugin-skills`)),
    codexPluginSkillDirs: rootsBySource.get("codex-plugin") || [],
    workspaceRoot,
  };
}
...
async function collectFilesystemSkills(roots, include) {
  ...
  for (const root of [
    roots.workspaceSkills,
    roots.projectAgentSkills,
    roots.personalAgentSkills,
    roots.openclawSkills,
    roots.bundledSkills,
    roots.pluginSkills,
    roots.extensionSkillsDir,
    roots.codexSkills,
    ...(roots.extraSkillDirs || []),
    ...(roots.codexPluginSkillDirs || []),
  ]) {
...
(async () => {
  ...
  const roots = await buildWorkspaceRoots();
