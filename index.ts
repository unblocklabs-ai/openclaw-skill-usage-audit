...
import {
  ...
  resolveCodexHome,
  resolveHomePath as resolveConfigHomePath,
} from "./skill-roots.mjs";
import {
...
interface RouterConfig {
  ...
  agentMaxSkillsToNudge?: unknown;
  subagentMaxSkillsToNudge?: unknown;
  cronMaxSkillsToNudge?: unknown;
  minScore?: unknown;
  agentMinScore?: unknown;
  subagentMinScore?: unknown;
  cronMinScore?: unknown;
  recencyWindow?: unknown;
  recencyFallbackMinutes?: unknown;
  lookbackMessages?: unknown;
  taskWindowMode?: unknown;
  discovery?: {
    workspace?: unknown;
    agentsProject?: unknown;
    agentsPersonal?: unknown;
    openclawManaged?: unknown;
    openclawBundled?: unknown;
    extraDirs?: unknown;
    openclawPluginSkills?: unknown;
    codex?: unknown;
    codexPlugin?: unknown;
    groupDepth?: unknown;
  };
...
type RouterTargetType = "agent" | "subagent" | "cron";
type RouterDecision = "nudged" | "skipped";
...
interface OpenClawSkillConfigSnapshot {
  agents?: {
...
interface DbPrepared {
  ...
  insertNudgeEvent: (params: Record<string, unknown>) => void;
  updateNudgeOutcome: (params: Record<string, unknown>) => void;
  refreshNudgeResourceCount: (params: Record<string, unknown>) => void;
  upsertSkillCatalog: (params: Record<string, unknown>) => void;
  insertRouterDecision: (params: Record<string, unknown>) => void;
  cleanupRouterDecisions: (params: Record<string, unknown>) => void;
  getRecentNudge: (params: Record<string, unknown>) => { id?: number } | undefined;
}

type NudgeOutcome = "nudged" | "opened" | "failed_open" | "ignored" | "unknown";
type NudgeEventType = "open_attempt" | "opened" | "failed_open" | "resources_used" | "coverage_gap" | "ignored" | "unknown";
...
interface PendingNudge {
  ...
  coverage: "strong" | "weak" | "insufficient";
  classificationReason?: string;
  resourceEventKeys: Set<string>;
  seenToolEventKeys: Set<string>;
  ended: boolean;
}
...
interface SharedShutdownParticipant {
  flush: (reason: string) => void | Promise<void>;
}
...
function clampIntConfig(value: unknown, fallback: number, min: number, max: number): number {
...
function parseStringConfig(value: unknown, fallback: string, allowed: Set<string>): string {
...
function inferSkillSource(skillPath: string): string {
  ...
  if (abs.includes(`${codexHome}/plugins/cache`) || abs.includes(`${codexHome}/plugins/cache/`)) return "codex-plugin";
  if (abs.includes(`${codexHome}/skills`) || abs.includes(`${codexHome}/skills/`)) return "codex";
  if (abs.includes(`${home}/.openclaw/extensions`) || abs.includes(`${home}/.openclaw/extensions/`)) return "extension";
  if (abs.includes(`${home}/.openclaw/plugin-skills`) || abs.includes(`${home}/.openclaw/plugin-skills/`)) return "extension";
  if (abs.includes(`${home}/.openclaw/skills`) || abs.includes(`${home}/.openclaw/skills/`)) return "managed";
...
function routerDiscoveryCacheKey(
  workspaceDir: string,
  config: OpenClawSkillConfigSnapshot | undefined,
...
function buildRouterDiscoveryConfig(discovery: RouterConfig["discovery"] | undefined): Record<string, unknown> {
  return {
    ...
    groupDepth: clampIntConfig(
      discovery?.groupDepth,
      DEFAULT_ROUTER_DISCOVERY_GROUP_DEPTH,
      0,
      MAX_ROUTER_DISCOVERY_GROUP_DEPTH,
    ),
  };
}
...
function getRoleFromMessage(message: unknown): string | undefined {
...
function wasSkillHandledRecently(messages: unknown[], skill: SkillCandidate, recencyWindow: number): boolean {
...
async function collectSkillCandidateFromDir(dirPath: string, source: string): Promise<SkillCandidate | undefined> {
...
async function collectSkillCandidatesFromRoot(root: string, source: string, groupDepth: number): Promise<SkillCandidate[]> {
  ...
  const walk = async (dirPath: string, remainingGroupDepth: number): Promise<void> => {
    const directSkill = await collectSkillCandidateFromDir(dirPath, source);
    if (directSkill) {
      out.push(directSkill);
      return;
    }
    if (remainingGroupDepth <= 0) return;

    let childEntries: Dirent<string>[];
    try {
...
async function readOpenClawConfigSnapshot(): Promise<OpenClawSkillConfigSnapshot | undefined> {
  try {
    const parsed = readOpenClawConfig() as unknown;
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) return undefined;
    return parsed as OpenClawSkillConfigSnapshot;
  } catch {
    return undefined;
  }
}

function shouldIncludeRouterCandidate(
  candidate: SkillCandidate,
  config: OpenClawSkillConfigSnapshot | undefined,
): boolean {
  if (isCandidateDisabledByEntries(candidate, config?.skills?.entries)) return false;
...
async function loadSkillCandidatesForWorkspace(
  workspaceDir: string,
  discovery: Record<string, unknown>,
): Promise<SkillCandidateCache> {
  const now = Date.now();
  const config = await readOpenClawConfigSnapshot();
  const bundleRoot = resolveBundledSkillsRoot({ moduleDir: __dirname } as any);
  ...
  const roots = await buildSkillRootSpecs({
    ...
    discovery,
  } as any) as Array<{ root: string; source: string }>;
  ...
  for (let r = 0; r < roots.length; r += 1) {
    ...
    for (const candidate of candidates) {
      if (!shouldIncludeRouterCandidate(candidate, config)) continue;
  ...
  const cacheEntry: SkillCandidateCache = { fetchedAt: now, candidates };
...
function scoreSkill(
  taskText: string,
  skill: { name: string; key?: string; description: string },
...
function formatNudge(skills: SkillCandidate[], nudgeIds: Map<string, string> = new Map()): string {
  ...
  for (const skill of skills) {
    ...
    if (nudgeId) lines.push(`    [skill-router:nudge ${nudgeId}]`);
...
function createPreparedStatements(db: SqliteBackend, log: { debug?: (msg: string) => void }): DbPrepared {
  ...
  return {
    ...
    getRecentNudge: (params) => {
      try {
        return getRecentNudge.get(params) as { id?: number } | undefined;
      } catch (err) {
...
async function initSqlite(path: string, log: { debug?: (msg: string) => void; info: (msg: string) => void; error: (msg: string) => void; }) {
...
export default definePluginEntry({
  ...
  register(api) {
    ...
    const routerTaskWindowMode = parseStringConfig(
      routerConfig.taskWindowMode,
      "recentMessages",
      new Set(["recentMessages", "latestUser", "promptOnly"]),
    );
    ...
    const routerObservabilityTopCandidates = clampIntConfig(
      routerObservability.topCandidates,
      DEFAULT_ROUTER_OBSERVABILITY_TOP_CANDIDATES,
      0,
      MAX_ROUTER_OBSERVABILITY_TOP_CANDIDATES,
    );
    const routerObservabilityRetentionDays = parseIntConfig(
      routerObservability.retentionDays,
      DEFAULT_ROUTER_OBSERVABILITY_RETENTION_DAYS,
      0,
    );
    ...
    const recentRouterNudges = new Map<string, RecentNudgeRecord>();
    ...
    let nudgeObservationChain = Promise.resolve<void>(undefined);
  ...
  function queueNudgeInsert(params: {
    ...
    outcome: NudgeOutcome;
    coverage: PendingNudge["coverage"];
    classificationReason?: string;
    skillRealPath?: string;
  }): void {
  ...
  function queueSkillCatalogUpserts(candidates: SkillCandidate[]): void {
  ...
  function queueNudgeOutcomeUpdate(
    nudge: PendingNudge,
    outcome: NudgeOutcome,
    reason: string,
    ...
    if (nudge.outcome === "opened") return;
    if (nudge.ended && nudge.outcome === outcome) return;
    ...
    scheduleDbWrite("failed updating nudge outcome", true, async () => {
      ...
      try {
        ...
        try {
          ...
          if (nudge.outcome === outcome) nudge.outcome = previousOutcome;
  ...
  function correlatedPendingNudges(event: RawEvent, ctx: RawContext | undefined): PendingNudge[] {
    const base = buildBase(event as { sessionId?: string; runId?: string }, ctx);
    return [...pendingNudges.values()].filter((nudge) => {
      if (nudge.ended) return false;
      if (nudge.runId && base.runId) return nudge.runId === base.runId;
      if (nudge.runId || base.runId) return false;
      if (nudge.sessionKey && base.sessionKey) return nudge.sessionKey === base.sessionKey;
  ...
  function queueRouterDecisionInsert(params: {
    ...
    reason: string;
    targetConfig?: { minScore: number; maxSkillsToNudge: number };
    taskText?: string;
    counts?: {
    ...
    topCandidates?: Array<{ skill: SkillCandidate; score: number; reason: string; status?: string }>;
  }): void {
    if (!routerObservabilityEnabled) return;
  ...
  function routerNudgeScope(ctx: RawContext | undefined, event: RawEvent | undefined): string {
    return buildScopeKeys(ctx, event).join("|");
  }

  function routerNudgeKey(scope: string, skill: SkillCandidate): string {
    return `${scope}:${skill.key.toLowerCase()}:${skill.name.toLowerCase()}`;
  }

  function wasNudgedRecentlyInMemory(
    scope: string,
  ...
  async function wasNudgedRecentlyInDb(
    event: RawEvent,
    ctx: RawContext | undefined,
    skill: SkillCandidate,
    turnNumber: number | null,
    ...
    const state = await ensureDbReady();
    if (!state.statements) return false;

    const base = buildBase(event as { sessionId?: string; runId?: string }, ctx);
    if (!base.runId && !base.sessionKey && !base.sessionId) return false;
  ...
  function rememberRouterNudge(scope: string, skill: SkillCandidate, turnNumber: number | null): void {
  ...
  async function observeNudgeToolCall(event: RawEvent, ctx: RawContext | undefined, occurredAt: string): Promise<void> {
    ...
    for (const nudge of nudges) {
      ...
      if (nudge.seenToolEventKeys.has(deliveryKey)) continue;
      nudge.seenToolEventKeys.add(deliveryKey);

      let matched = false;
      for (const access of extracted.accesses) {
        const accessKind = await classifyAccessForSkill(
          access.path,
          nudge.skillPath,
          nudge.skillRealPath,
          access.cwd || toStringLike(ctx?.workspaceDir),
        );
  ...
  function finishNudgesForAgentEnd(event: RawEvent, ctx: RawContext | undefined): void {
    const nudges = correlatedPendingNudges(event, ctx);
    for (const nudge of nudges) {
  ...
  function finishNudgesForSessionEnd(event: RawEvent, ctx: RawContext | undefined): void {
    const base = buildBase(event as { sessionId?: string; runId?: string }, ctx);
    for (const nudge of [...pendingNudges.values()]) {
      const sameSession = Boolean(
        (nudge.sessionKey && base.sessionKey && nudge.sessionKey === base.sessionKey)
  ...
  async function maybeNudgeSkills(event: RawEvent, ctx: RawContext): Promise<{ prependContext: string } | undefined> {
    ...
    const shouldRoute =
      (isRegularAgent && routerTargetAgent) ||
    ...
    const eventMessages = (event as RawEvent).messages;
    const messageList: unknown[] = Array.isArray(eventMessages) ? eventMessages : [];
    ...
    const candidateByName = buildSkillIdentityMap(candidates) as Map<string, SkillCandidate>;
    ...
    const wasSuppressedRecently = async (skill: SkillCandidate) => {
      if (wasSkillHandledRecently(messageList, skill, routerRecencyWindow)) return true;
      if (wasNudgedRecentlyInMemory(nudgeScope, skill, turnNumber, routerRecencyWindow, routerRecencyFallbackMinutes)) return true;
      return wasNudgedRecentlyInDb(event, ctx, skill, turnNumber, routerRecencyWindow, routerRecencyFallbackMinutes);
    };
    let scoredForDecision: Array<{ skill: SkillCandidate; score: number; reason: string; status?: string }> = [];
    ...
    for (const override of routerOverrides) {
      ...
      for (const skillName of override.skills || []) {
        ...
        if (!skill) continue;
        if (isSkillBlocked(skill, blocklist)) continue;
        const selectedKey = canonicalSkillId(skill);
        if (seen.has(selectedKey)) continue;
        if (await wasSuppressedRecently(skill)) continue;

        hasOverride.push(skill);
        seen.add(selectedKey);
        if (hasOverride.length >= targetMaxSkillsToNudge) break;
      }
    ...
    if (hasOverride.length) {
      ...
      for (const row of eligibleScored) {
        ...
        if (suppressed) {
          ...
          if (decisionRow) decisionRow.status = "suppressed_recently";
          continue;
        }
        ...
        if (decisionRow) decisionRow.status = "selected";
        seen.add(key);
        if (selected.length >= targetMaxSkillsToNudge) break;
      }
    }
    ...
    const taskExcerpt = routerObservabilityIncludeTaskExcerpt
      ...
      : null;
    const nudgeIds = new Map<string, string>();
    for (const skill of selected) {
      ...
      const coverage: PendingNudge["coverage"] = hasStrongCorrelation ? "strong" : hasWeakCorrelation ? "weak" : "insufficient";
      const initialOutcome: NudgeOutcome = hasWeakCorrelation || hasStrongCorrelation ? "nudged" : "unknown";
      ...
      if (!pending.ended) pendingNudges.set(nudgeId, pending);
  ...
  api.on("session_end", async (event, ctx: RawContext) => {
    ...
    finishNudgesForSessionEnd(event as RawEvent, ctx);
  ...
  api.on("before_tool_call", async (event, ctx: RawContext) => {
    ...
    const scopeKeys = buildScopeKeys(ctx, event);
    const isSkillRead = toolName === "read" && extractSkillPathFromParams(params) !== undefined;
    if (!isSkillRead) attachToolCall(scopeKeys, event as RawEvent);
  });
  ...
  api.on("after_tool_call", async (event, ctx: RawContext) => {
    ...
    const params = ((event as RawEvent).params as Record<string, unknown>) || {};
    const scopeKeys = buildScopeKeys(ctx, event);
    let linked = detachToolCall(event as RawEvent, scopeKeys);
    const failed = isToolCallFailure(event as RawEvent);
    ...
    if (!failed && toolName === "read" && skillPath) {
      ...
      linked = detachToolCall(event as RawEvent, scopeKeys) || execution;
    }
    nudgeObservationChain = nudgeObservationChain
      .then(() => observeNudgeToolCall(event as RawEvent, ctx, now))
      .catch((error) => {
  ...
  api.on("agent_end", async (event, ctx: RawContext) => {
    await nudgeObservationChain;
    finishNudgesForAgentEnd(event as RawEvent, ctx);
  });
  ...
  function finalizeAllPendingNudges(reason: string): void {
    for (const nudge of [...pendingNudges.values()]) {
      if (nudge.outcome === "nudged") {
        nudge.coverage = "insufficient";
